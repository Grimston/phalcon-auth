...
namespace Grimston\Security\PhalconAuth;

use Grimston\Security\PhalconAuth\Contracts\AuthableInterface;
use Grimston\Security\PhalconAuth\Contracts\RememberableAuthableInterface;
use Grimston\Security\PhalconAuth\Exceptions\EntityBannedException;
use Grimston\Security\PhalconAuth\Exceptions\EntityNotFoundException;
use Grimston\Security\PhalconAuth\Exceptions\RememberMeTokenExpired;
use Grimston\Security\PhalconAuth\Exceptions\WrongCredentialsException;
...
class Auth extends Injectable
{
    ...
    public function register(
        ...
        try {
            ...
            throw new RuntimeException('Caught RuntimeException in ' . __METHOD__ . ' at line ' . __LINE__
                                       . ': error creating new authable model.');
        }
        ...
        if (count($entity->getMessages())) {
            ...
            throw new RuntimeException('Caught RuntimeException in ' . __METHOD__ . ' at line ' . __LINE__
                                       . ': error creating new authable model: ' . $messages);
        }
     ...
     */
    public function confirm(int $idEntity, string $confirmationCode)
    {
        // Retrieve the entity
     ...
     * @throws EntityNotFoundException
     */
    public function getResetPasswordToken(int $idEntity)
    {
        // Retrieve the entity
     ...
     * @throws UnexpectedValueException
     */
    public function resetPassword(int $idEntity, string $resetToken, string $newPassword)
    {
        // Retrieve the entity
     ...
     */
    public function attemptLogin(string $email, string $password, $saveSession = true, $rememberMe = false)
    {
